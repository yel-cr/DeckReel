#!/usr/bin/env python3
"""
DeckShot - Steam Deck スクリーンショット マネージャー
...
HOST = "127.0.0.1"
PORT = 8745
CONFIG_DIR = Path.home() / ".config" / "deckshot"
    ...
    def resolve(self, app_id):
        ...
        for attempt in range(3):
            try:
                ...
                req = urllib.request.Request(url, headers={"User-Agent": "DeckShot/1.3"})
...
<div class="topbar">
  <div class="topbar-logo">
    ...
    <div class="logo-wordmark">DECKSHOT</div>
  </div>
  ...
  <div class="content" id="contentArea">
    ...
    <div class="grid-area" id="gridArea">
      <div class="welcome" id="welcomePanel">
        <div class="welcome-title">DECKSHOT</div>
...
function updateTicker(){
  ...
  const sy=games.reduce((s,g)=>s+g.synced,0);
  const items=[
    {t:'DeckShot v'+appVersion,hi:false},
...
class DeckShotHandler(BaseHTTPRequestHandler):
    config = None
    resolver = None
    tracker = None
    scanner = None
...
def main():
    ...
    DeckShotHandler.config = config
    DeckShotHandler.resolver = resolver
    DeckShotHandler.tracker = tracker
    DeckShotHandler.scanner = scanner
    DeckShotHandler.sync_engine = sync_engine
    ...
    server = HTTPServer((HOST, PORT), DeckShotHandler)
    DeckShotHandler._server_ref = server
    DeckShotHandler.touch_heartbeat()
    ...
    def heartbeat_watchdog():
        while True:
            time.sleep(3)
            elapsed = DeckShotHandler.seconds_since_heartbeat()
